...
- The other six run as **QA / validation when needed**, with **five hard checkpoints** that block the pipeline (full reference: `design-system/rules/quality-gates.md`): **Design System** (before implementation), **Typography** (IBM Plex Sans Arabic + scale), **Component Quality** (criteria + right component), **Anti-Generic** (page not bland/generic — `anti-bland-ui-rules.md`), and **Accessibility** (before delivery). The mandatory default base is `default-design-system-standard.md`; delivery conditions in `delivery-contract.md`.
