...
- The other six run as **QA / validation when needed**, with three hard checkpoints that block the pipeline: the **Design System Gate** (before implementation is valid), the **Component Quality Gate** (every component meets its acceptance criteria + is the right choice — `design-system/rules/component-quality-gate.md`), and the **Accessibility Gate** (before delivery is acceptable).
