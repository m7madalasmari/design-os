...
- The other six run as **QA / validation when needed**, with **six hard checkpoints** that block the pipeline (full reference: `design-system/rules/quality-gates.md`): **Design System** (before implementation), **Typography** (IBM Plex Sans Arabic + scale), **Component Quality** (criteria + right component), **Visual Polish** (rendered output not native/default/generic — `visual-polish-gate.md`), **Anti-Generic** (page not bland — `anti-bland-ui-rules.md`), and **Accessibility** (before delivery). The mandatory default base is `default-design-system-standard.md`; delivery conditions in `delivery-contract.md`.
