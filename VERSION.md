...
- `design-system/` **(v2.3.0)** — tokens.css (executable, **+`--brand-*` ramp tier** +secondary/breakpoints/inverse) + foundations (+icon-system +color-system +component-tokens +token-architecture +typography-system +component-visual-baseline) + components (+INDEX, **31 components** — +avatar +detail-view +sidebar +app-header) + rules (+component-quality-gate +anti-bland-ui-rules +quality-gates +visual-polish-gate) + themes (seed→ramp). **6 بوّابات.**
...
Git tag for this release: **`v2.3.0`**.
