...
7. **Map components** to `design-system/components/INDEX.md`. A missing component/token is documented in `design-system/` **first**, then used. No new component without checking INDEX. **← Design System Gate** ([`ROLES.md`](ROLES.md)): implementation is not valid until this passes (tokens-only, all blocks mapped, anything new documented first).
8. **Implement** — Tailwind-first, semantic token classes, logical RTL utilities, WCAG AA. No arbitrary values (Pixel Clone excepted, documented); token-only `var()` exceptions documented (`02`). SPA tasks also obey `09`. **← Component Quality Gate** (`design-system/rules/component-quality-gate.md`): each component must meet its acceptance criteria (states · RTL · no clutter · one primary · token bindings) **and** be the right component for the job — else it is not done.
9. **QA + role sweep** (`03`): System QA + RTL QA (`04`) + **Accessibility QA (`06`, every interface)** + Visual-Polish QA (`05`). Run the **Review-phase [`ROLES.md`](ROLES.md) sweep** here — Visual Designer · Accessibility · QA · UX Writer each emit one `pass`/`flag` line; Handoff Lead closes at step 11. **← Accessibility Gate** ([`ROLES.md`](ROLES.md)): delivery is not acceptable until the WCAG-AA floor passes (contrast · keyboard · focus · ARIA · reduced motion). **Pixel QA only if a reference image exists.** SPA tasks also run the **Engineering DoD** gates (`07`).
